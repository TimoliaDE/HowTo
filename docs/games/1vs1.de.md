...
Spiele alleine oder im Team gegen deine Gegner in 1vs1. Wähle eigene oder andere Kits und finde in fairen Kämpfen heraus, wer die oder der Bessere ist.
...
Arbeite dich in unserem Ranked-System die Karriereleiter hinauf und werde die oder der höchstrangige Spieler:in des Servers.
Fordere andere Spieler:innen oder Teams heraus oder nimm an einem Turnier teil und beweise dich gegenüber anderen Spielerinnen und Spielern.
...
- Mit einem Linksklick auf eine:n Spieler:in wird diese:r zum Duell herausgefordert, ein Rechtsklick lädt diese:n in ein Team ein.
...
- Mit einem Rechtsklick auf die oder den Kämpfer:in beobachtest du den Kampf aus seiner Perspektive.
...
- Um noch schneller noch fairere Matches zu finden, kannst du in der globalen Warteschlange gegen Spieler:innen auf allen Servern spielen.
...
Arbeite dich in unserem Ranked-System die Karriereleiter hinauf und werde die oder der höchstrangige Spieler:in des Servers. 
Insgesamt gibt es 17 Ränge, die es zu erreichen gilt. Jede:r Spieler:in besitzt in 1vs1 einen zugewiesenen Wert, die Ranked Punkte. 
Bei einem Sieg gegen deine:n Gegner:in erhöhen sich diese, verlierst du ein Spiel, werden dir Punkte abgezogen. Dein Rang bestimmt sich aus der Höhe deiner Punkte. 
Die genauen Punktegrenzen für die einzelnen Ränge kannst du ingame mit dem Befehl `/ranks` aufrufen. Beachte, dass die besten Ränge nur von einer bestimmte Anzahl an Spielerinnen und Spielern gleichzeitig besetzt sein können. 
So können nur 3 Spieler:innen gleichzeitig Rang 17 sein, auch wenn mehr als 3 Spieler:innen >8500 Punkte haben. Alle Infos zu deinem aktuellen Rang und deinen Punkteständen findest du unter `/season`.
...
Du startest bei jedem Kit mit 0 Punkten. Für gewonnene Spiele bekommst Punkte gutgeschrieben, für verlorene abgezogen. Neben den Ranked Punkten hat jede:r Spieler:in für jedes Kit auch einen Elo Wert. 
Dieser wird dir nicht angezeigt, sondern versucht im Hintergrund deine Spielstärke zu ermitteln. Wie groß die Differenz zwischen deinen Punkten und deiner Elo sowie der Elo deiner oder deines Gegnerin oder Gegners ist, bestimmt, wie viele Punkte du für Sieg oder Niederlage dazugewinnst bzw. verlierst.
...
### Seasons
Am Ende jeder Season werden alle Ranked Ranglisten und Punkte zurückgesetzt, die Top Spieler:innen der letzten Season werden in der PvP Lobby angezeigt. Die gesamten Statistiken (auch deine) der letzten Seasons kannst du ingame mit `/season [Nummer]` aufrufen. 
...
## Winstreaks
Verdiene mit Winstreaks zusätzlich Pixel und kaufe dir mit diesen tolle Items im Pixelshop. Spieler:innen mit [Premium](/ranks/premium/) erhalten mehr Pixel.
...
Das Turnier wird gleichzeitig von einer oder einem Streamer:in live übertragen. Die jeweiligen Streamer:innen und Kits werden auf [Twitter](https://twitter.com/TimoliaTeam) und im [Timolia-Discord](/discord/) angekündigt.
Die oder der Gewinner:in hat die Möglichkeit, den [MVP-Rang](/ranks/premium/) für einen Monat zu gewinnen. 
Nimmt sie oder er das Risikomatch gegen die oder den Streamer:in an und gewinnt es, kann sie oder er sogar 2 Monate gewinnen. Verliert sie oder er allerdings, bekommt sie oder er nur einen halben Monat.
Außerdem erhalten die Top 3 eine Medaille sowie einen entsprechenden Subtitle. Alle Spieler:innen, die es bis auf den Finalserver schaffen, erhalten zudem 3 Tage Pro!
...
## Premium-Features
- Während normale Spieler:innen lediglich ein Kit speichern können, stehen Premium-Spielerinnen und Premium-Spielern je nach Rang bis zu 5 Slots zur Verfügung. Bei der Mapauswahl haben normale User Zugriff auf einen und Premium-Spielerinnen und Premium-Spieler je nach Rang bis auf 5 Slots.
- Des Weiteren können Premium-Spielerinnen und Premium-Spieler Turniere (`/create`) und Teams mit mehr als zwei Spielerinnen und Spielern erstellen. Einige Ränge können sogar serverweite 1vs1-Turniere starten (`/create`).
- Premium-Spielerinnen und Premium-Spieler erhalten bis zu drei mal mehr Pixel.
- Setze dir im Kitbereich mit `/hat` Items in deiner Hand auf den Kopf.
- Wähle mit `/team setcolor` deine Teamfarbe aus.
- Fordere mit `/bestofx Spieler> <Anzahl>` eine:n Spieler:in zu einem Best of 3 oder Best of 5 heraus.
- Werde mit `/togglerank` als normale:r Spieler:in angezeigt.
...
| Befehl | Funktion |
| ------ | -------- |
| `/challenge <Spieler:in>`, `/c`    | Fordert eine:n Spieler:in zu einem Duell heraus |
| `/compare <Name> <Name>`        | Zeigt die Kampfstatistik zweier Spieler:innen |
...
| `/kit <Name(:1/2/3/4/5)>`       | Wählt das Kit einer oder eines anderen Spielerin oder Spielers aus |
| `/kitinfo`                      | Zeigt Infos über das ausgewählte Kit |
| `/kits`                         | Listet dir alle von Timolia erstellten Kits auf |
| `/lastinv <Spieler:in>`, `/inv`    | Zeigt das letzte Inventar einer oder eines Spielerin oder Spielers |
| `/leave`, `/l`                  | Verlässt ein Turnier bzw die Warteschlange |
| `/liveinv <Spieler:in>`, `/liveinventory` | Zeigt das aktuelle Inventar einer Spielerin oder eines Spielers |
...
| `/spectate <Spieler:in>`, `/spec`  | Lässt dich den Kampf einer Spielerin oder eines Spielers beobachten |
| `/stats`                        | Zeigt dir deine Stats an |
| `/stats <Name>`                 | Zeigt dir die Stats einer Spielerin oder eines Spielers an |
...
| `/team invite <Name>`           | Lädt eine:n Spieler:in in dein Team ein |
| `/team kick <Name>`             | Kickt eine:n Spieler:in aus deinem Team |
...
| `/bestofx <Spieler:in> <Anzahl>`   | Fordert eine:n Spieler:in zu einem Best of 3 oder Best of 5 heraus |
