...
const MOJANG_API_URL = "https://playerdb.co/api/player/minecraft/";
...
export async function getNameFromMojang(uuid) {
  try {
    ...
    const response = await fetch(`${MOJANG_API_URL}${formattedUUID}`, {
      headers: {
        "User-Agent": "TimoliaHowTo/1.0 (Contact: info@timolia.de)",
        Accept: "application/json",
        Why: "This request is made to fetch the new player name from Mojang API",
      },
    });
...
export async function updateTeamlerNames(teamlers) {
  ...
  for (let i = 0; i < teamlers.length; i++) {
    ...
    if (name) {
      teamler.name = name;
      console.log(
        `[${i + 1}/${teamlers.length}] Updated: ${name} (${teamler.uuid})`,
      );
    } else {
      console.warn(
        `[${i + 1}/${teamlers.length}] Could not fetch name for ${teamler.uuid}, keeping as "${teamler.name}"`,
      );
    }
